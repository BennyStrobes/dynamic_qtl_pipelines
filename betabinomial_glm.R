...
betaBinomialGLM=function(ys,ns,xFull,xNull,concShape=1.0001,concRate=1e-4,...) {
  ...
  fit_full <- optimizing(BETABINOMIAL_GLM, data=datFull, init=initFull, algorithm="BFGS", hessian=T, as_vector=F, verbose=F)
...
testbbglm=function() {
  ...
  old_coef=get_glm_coefs(anova_result$fit_full)
  to_keep=ns>0
  ns=ns[to_keep]
  ys=ys[to_keep]
  xNull=xNull[to_keep,,drop=F]
  xFull=xFull[to_keep,,drop=F]
  anova_result=betaBinomialGLM(ys,ns,xFull,xNull)
  get_glm_coefs(anova_result$fit_null)
  new_coef=get_glm_coefs(anova_result$fit_full)
  stopifnot(all(old_coef==new_coef))
}
