...
# Load in results
results_1 <- paste0(results_dir, "te_log_linear_environmental_variable_time_steps_optimizer_LBFGS_genotype_dosage_covariate_method_none_permutation_scheme_none_permute_False_merged_dynamic_qtl_results.txt")
results_2 <- paste0(results_dir, "te_log_linear_environmental_variable_time_steps_optimizer_LBFGS_genotype_dosage_covariate_method_none_permutation_scheme_sample_null_permute_True_merged_dynamic_qtl_results.txt")
results_3 <- paste0(results_dir, "te_log_linear_environmental_variable_time_steps_optimizer_LBFGS_genotype_dosage_covariate_method_none_permutation_scheme_shuffle_all_permute_True_merged_dynamic_qtl_results.txt")
results_4 <- paste0(results_dir, "te_log_linear_environmental_variable_time_steps_optimizer_LBFGS_genotype_dosage_covariate_method_none_permutation_scheme_shuffle_lines_permute_True_merged_dynamic_qtl_results.txt")
...
type <- c(rep("real",length(pvalz_1)), rep("bootstrap: sample null",length(pvalz_2)), rep("perm: shuffle all",length(pvalz_3)), rep("perm: shuffle lines",length(pvalz_4)))
...
output_file <- paste0(qtl_visualization_dir, "qq_plot_te_log_linear_environmental_variable_time_steps_genotype_dosage_compare_perm_schemes.png")
